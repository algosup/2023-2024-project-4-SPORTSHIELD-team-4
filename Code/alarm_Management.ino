...
void onWriteStopAlarm(BLEDevice central, BLECharacteristic characteristic) {
  Serial.println("onWriteStopAlarm appelée"); // Ajoutez cette ligne
  if (StopAlarmCharacteristic.value()) {
    Serial.println("Arrêt de l'alarme demandé"); // Ajoutez cette ligne
    stopAlarm();
...
void PulseBuzzer(int repetitions, unsigned long durationOn, unsigned long durationOff) {
    ...
    // Vérifie si l'alarme doit être arrêtée
      Serial.println("PulseBuzzer appelée"); // Ajoutez cette ligne
    if (alarmShouldStop) {
    ...
    if (currentRep < repetitions) {
        if ((buzzerState == LOW && currentMillis - previousMillis >= durationOn) ||
            (buzzerState == HIGH && currentMillis - previousMillis >= durationOff)) {
